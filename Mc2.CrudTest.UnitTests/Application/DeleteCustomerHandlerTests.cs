using Mc2.CrudTest.Application.Commands;
using Mc2.CrudTest.Application.Commands.Handlers;
using Mc2.CrudTest.Application.Exceptions;
using Mc2.CrudTest.Domain.Entities;
using Mc2.CrudTest.Domain.Factories;
using Mc2.CrudTest.Domain.Repositories;
using Mc2.CrudTest.Shared.Abstraction.Commands;
using NSubstitute;
using Shouldly;

namespace Mc2.CrudTest.UnitTests.Application;

public class RemoveCustomerHandlerTests
{

    Task Act(RemoveCustomerCommand command)
            => _commandHandler.HandleAsync(command);

    [Fact]
    public async Task HandleAsync_Throws_CustomerNotFoundException()
    {
        _repository.GetAsync(_removeCustomerCommand.Id).Returns(default(Customer?));

        var exception = await Record.ExceptionAsync(() => Act(_removeCustomerCommand));

        exception.ShouldNotBeNull();
        exception.ShouldBeOfType<CustomerNotFoundException>();
    }

    [Fact]
    public async Task HandleAsync_Calls_Repository_Success()
    {
        var customer = _factory.Create(_removeCustomerCommand.Id, "hamed", "lohi", new DateTime(1990, 7, 8),
            "09149012500", "lohi.hamed@gmail.com", "123456789");
        _repository.GetAsync(_removeCustomerCommand.Id).Returns(customer);

        //_phoneService.ValidatePhoneNumberAsync(_createCustomerCommand.PhoneNumber).Returns(string.Empty);

        var exception = await Record.ExceptionAsync(() => Act(_removeCustomerCommand));

        exception.ShouldBeNull();
        await _repository.Received(1).DeleteAsync(Arg.Any<Customer>());
    }

    #region ARRANGE

    private readonly ICommandHandler<RemoveCustomerCommand> _commandHandler;
    private readonly ICustomerRepository _repository;
    private readonly ICustomerFactory _factory;
    private RemoveCustomerCommand _removeCustomerCommand;

    public RemoveCustomerHandlerTests()
    {
        _repository = Substitute.For<ICustomerRepository>();
        _commandHandler = new RemoveCustomerHandler(_repository);
        _factory = new CustomerFactory();
        _removeCustomerCommand = new RemoveCustomerCommand(Guid.NewGuid());
    }

    #endregion

}